...
metabo_collate <- function(path = "~/Desktop")
{
  ## ----------------------------------------------------------------------
  ## read file
...
metabo_collate_pathway <- function(pathway)
{
...
.meta_kegg_check_kegg_get <- function(id_set)
{
...
.meta_kegg_check_get_name <- function(list, id = "ENTRY", name = "NAME")
{
  id <- list[[id]][1]
...
## ------------------------------------- 
.meta_sort_uniq_df <- function(df, col, pattern_set)
{
  levels <- df[[col]] %>% 
